...
class AFXLoadingWindow(AFXWindow):
	def __init__(self):
		...
		self.status_text = gtk.Label("Status...")
		#self.main_box.pack_start(self.main_box.status_text, gtk.FALSE, gtk.FALSE, 0)
		self.add(self.status_text)
		self.status_text.modify_fg(gtk.STATE_NORMAL, AFX.RGBColor(255, 255, 255))
		self.status_text.show()


	def update_status(self, text):
		self.status_text.set_markup("<span size='xx-large' weight='bold'>%s</span>" % text)
		self.status_text.queue_draw()
		self.queue_draw()
		AFX.update_ui()
